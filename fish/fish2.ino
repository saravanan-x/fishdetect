...
// MJPEG video stream (replaces old single-frame capture)
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    while (true) {
        ...
        captureCount++;   // keep original counter (now counts streamed frames)
...
void startServer() {
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        Serial.println("HTTP server started → only /sensors (JSON) + /stream (MJPEG video)");
    } else {
